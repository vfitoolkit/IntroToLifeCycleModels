%% Life-Cycle Model A7: Three markov (z) and three iid (e) shocks
% We now have six exogenous states, three markovs (z1, z2, z3) and three
% i.i.d. (e1,e2,e3).
%
% So the inputs to ReturnFn and FnsToEvaluate will have (...,z1,z2,z3,e1,e2,e3,...)
%
% All shocks are independent, so first we can discretize all six
% separately.
%
% We construct the grids by stacking the columns of each of the shocks, to
% get z_grid and e_grid.
%
% We do the transition matrices in stages.
% First we combine the first two shocks (in the same we did in A5i-A6)
% pi_z1z2=kron(pi_z2,pi_z1); (note reverse order)
% And then we combine this with the third shock
% pi_z=kron(pi_z3,pi_z1z2); 
% Notice that as always we use the reverse order in kron().
%
...
% size(e_grid)=sum(n_e)-by-1
% size(pi_z)=prod(n_z)-by-prod(n_z)
% size(pi_e)=prod(n_e)-by-1
%
% With so many exogenous shocks this problem is pretty massive, so we use
% vfoptions.lowmemory=1 which uses more loops and less parallelization (so
% slower, but less memory). I also turn on vfoptions.verbose=1 so you can
% see how it is going.
%
% All of this can be done using joint-grids, or with age-dependent shocks.
...
% Three stochastic exogenous markov state variables: z1,z2 and z3, all are shocks to labor efficiency units
% Three stochastic exogenous i.i.d. state variables: e1,e2 and e3, all are shocks to labor efficiency units
% Age: j
...
n_z=[7,5,3];% Exogenous labor productivity units shock, three markov
n_e=[7,5,3]; % Exogenous labor productivity units shock, three i.i.d.
...
% Exogenous shock process, z3: AR1 on labor productivity units
Params.rho_z3=0.1;
Params.sigma_epsilon_z3=0.05;
% Exogenous shock process, e1: i.i.d. log-normal on labor productivity units
Params.mew_e1=0;
Params.sigma_epsilon_e1=0.03;
% Exogenous shock process, e2: i.i.d. normal on labor productivity units
Params.mew_e2=1.3;
Params.sigma_epsilon_e2=0.03;
% Exogenous shock process, e3: i.i.d. uniform on labor productivity units
Params.e3_min=0.5;
Params.e3_max=1.5;
...
%% Dealing with three wo markovs: grid and transition matrix, z_grid and pi_z
% First, just discretize each of our three AR(1) processes
...
% Discretize the AR(1) process z3
[z3_grid,pi_z3]=discretizeAR1_FarmerToda(0,Params.rho_z3,Params.sigma_epsilon_z3,n_z(3));
z3_grid=exp(z3_grid); % Take exponential of the grid
[mean_z3,~,~,~]=MarkovChainMoments(z3_grid,pi_z3); % Calculate the mean of the grid so as can normalise it
z3_grid=z3_grid./mean_z3; % Normalise the grid on z3 (so that the mean of z3 is exactly 1)
...
pi_z1z2=kron(pi_z2,pi_z1); % note reverse order
% then combine result with the third
pi_z=kron(pi_z3,pi_z1z2); % note reverse order

%% Dealing with three i.i.d.: grid and transition matrix, e_grid and pi_e
% First, just discretize each of our three i.i.d. normal processes
% Discretize the i.i.d. log-normal e1
...
% Discretize the i.i.d. normal e2
[e2_grid,pi_e2]=discretizeAR1_FarmerToda(Params.mew_e2,0,Params.sigma_epsilon_e2,n_e(2));
pi_e2=pi_e2(1,:)'; % i.i.d., so just use first row of matrix (and transpose it to column)

% Discretize the i.i.d. uniform e3
e3_grid=linspace(Params.e3_min,Params.e3_max,n_e(3))';
pi_e3=ones(n_e(3),1)/n_e(3); % equal probability on each point

% Now, we put together the three grids, as a stacked column
e_grid=[e1_grid; e2_grid; e3_grid];

% Next, we put together pi_e
% combine the first two
pi_e1e2=kron(pi_e2,pi_e1); % note reverse order
% then combine result with the third
pi_e=kron(pi_e3,pi_e1e2); % note reverse order
...
% Use 'LifeCycleModelA7_ReturnFn', which has thee markov and three i.i.d. exogenous states
ReturnFn=@(h,aprime,a,z1,z2,z3,e1,e2,e3,w,sigma,psi,eta,agej,Jr,pension,r,kappa_j,warmglow1,warmglow2,warmglow3,beta,sj)... 
    LifeCycleModelA7_ReturnFn(h,aprime,a,z1,z2,z3,e1,e2,e3,w,sigma,psi,eta,agej,Jr,pension,r,kappa_j,warmglow1,warmglow2,warmglow3,beta,sj)
...
% V is now (a,z1,z2,z3,e1,e2,e3,j). One dimension for each state variable.
% Compare
size(V)
% with
[n_a,n_z,n_e,N_j]
...
% endogenous state variable 'a', one for each exogenous markov state
% variable, 'z', and one for each exogenous i.i.d state variable 'e'
