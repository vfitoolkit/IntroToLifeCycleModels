% Life-Cycle Model A9:  Two age-dependent semi-exogenous (semiz) shocks
% 
% Model has two semi-exogenous states, n and work, that represent the
% number of children and whether household has a job. The transitions of
% these semi-exogenous states are controlled by two decision variables, f
% and search, which represent fertility decision and a job search decision.
% 
...
%
% Three decision variable: h, f, and search. labour hours worked, fertility decision, and job search intensity
...
% Age: j
%
% 'Last' two decision variable influences the semi-exogenous states: f and search
% Two semi-exogenous states: children (number of), and work (=1 if have job, =0 if does not have job)
%
% Note: Only people with work=1 can choose their h, labor hours worked.
...
% Grid sizes to use
n_d=[21,2,5]; % Endogenous labour choice (fraction of time worked), fertility decision
n_a=201; % Endogenous asset holdings
n_semiz=[3,2]; % number of children, work (whether or not employed)
...
f_grid=[0;1];
% Grid for job search effort
search_grid=linspace(0,1,n_d(3))';
% Switch into toolkit notation
d_grid=[h_grid; f_grid; search_grid]; % stacked column vector

%% Set up for the semi-exogenous states
% Grids for number of children
...
title('Life Cycle Profile: Employment status (work)')
...
pi_semiz_J=zeros([n_semiz,n_semiz,n_d(2),n_d(3),N_j]); % Note that endogenous state is the first, then the conditional transition matrix for shocks
for f_c=1:n_d(2)
    for search_c=1:n_d(3)
        for n_c=1:n_semiz(1)
            for work_c=1:n_semiz(2)
...
pi_semiz_J2=reshape(pi_semiz_J,[prod(n_semiz),prod(n_semiz),n_d(2),n_d(3),N_j]);
% Make sure the 'rows' sum to one
for jj=1:N_j
    for f_c=1:n_d(2)
        for search_c=1:n_d(3)
            temp=sum(pi_semiz_J2(:,:,f_c,search_c,jj),2);
...
% Conditional on the decision variables and age, this just looks like a standard markov transition matrix
pi_semiz_J2(:,:,1,1,1) % arbitrarily use the first index for f work and j
