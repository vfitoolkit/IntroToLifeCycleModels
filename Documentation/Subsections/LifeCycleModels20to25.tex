...
To be able to do this we need to use a non-separable utility function.\footnote{Non-separable utility was discussed Assignment 2 (just after Life-Cycle model 10).} Conceptually the reason is simple enough. If incomes grow (which is the point of our deterministic productivity growth $g$) then people will consume more and so the marginal utility of consumption falls. Then separable utility implies that equating marginal utilities therefore requires a falling marginal utility of leisure, which means ever increasing leisure. So separable utility together with growing incomes drives hours worked to decrease towards zero. With non-separable utility the marginal utility of leisure is increasing in consumption (this is meaning of non-separable) and so (optimal) leisure does not continually increase as consumption increases, and so hours worked to not tend to zero.
...
Let's extend Life-Cycle Model 9 to include deterministic income growth, as part of which we will have to switch to a non-separable utility function. To make things simple I am going to index pensions to the growth of wages.\footnote{This is not necessary, just makes the renormalized model a bit simpler.} So we start with the household problem,
\begin{eqnarray*}
...
&& log(z_j)=\rho_z log(z_{j-1}) + \epsilon, \; \epsilon \sim N(0,\sigma_{\epsilon,z}^2) \\
\end{eqnarray*}
note that we now just have $w_1$ and $pension_{Jr}$. Notice also the $warmglow(\hat{a}_{j+1} (1+g)^j)$; we could just take the $(1+g)^j$ out of the warmglow function as really it is just equivalent changing two of the warm-glow parameters, $wg1$ and $wg2$. When coding we will take advantage of this to rewrite the warm-glow as $(1-s_j)\beta (1+g)^{-1}  \mathbb{I}_{(j>=Jr+10)} warmglow(\hat{a}_{j+1} (1+g))$, as mentioned this is really just reparametrizing, not changing anything of substance.
...
\subsection{Life-Cycle model 23: Permanent Types: Solving fixed-types}
...
Notice that the value function now depends on $i$: $V_i$. So we are now essentially solving $N\_{}i$ separate problems.
...
\subsection{Life-Cycle model 24: Using Names for Permanent Types: patient and impatient}
...
Rather than use $N\_{}i=2$, like we did in Life-Cycle model 23, we can instead give the agents names using $Names\_{}i=\{'patient','impatient'\}$. The PType commands will automatically use these names for all outputs. We set the different parameter values as $Params.beta.patient=0.96$ and $Params.beta.impatient=0.9$. After this, other than using $Names\_{}i$ everywhere we had $N\_{}i$ in Life-Cycle model 23, there is no other change we need to make.
...
\subsection{Life-Cycle model 25: More Permanent Types}
...
The following Life-Cycle model 26 also uses permanent types, for the same purpose of modelling a fixed-effect as was seen in Life-Cycle model 23.
