...
We will set this up in the code and then use the command 'ValueFnIter\_{}FHorz\_{}Case1()' to solve to get both the value function and the policy function. The value function tells us the value at each point on the grid (currently the only variable the value function depends on is $j$). It also gives us the 'policy function', which is the optimal decision the agent makes, in our case this will just be the choice of h. The 'Policy' is the index for the grid point for optimal decision 'h', rather than the value of 'h'. As a user you can pass Policy to various toolkit commands, and if you need the values of the policy you can use $PolicyInd2Val\_{}FHorz()$ as seen later in this code. We will do two brief examples, one later in this code, and one in Life-Cycle Model 3, that show how PolicyValues gets created from Policy in simple models. In more advanced models the content of Policy gets complicated, but the toolkit will always handle that for you without a real need for you to be able to interpret the content, just switch it to PolicyValues using $PolicyInd2Val\_{}FHorz()$ if you ever want to look at the optimal policies as that is always much easier to understand.
...
Because of how the codes work, before we can calculate the life-cycle profiles we have to create something called the 'stationary distribution'.\footnote{The stationary distribution is not needed for the specific life-cycle profiles we need in this model, but is necessary for other life-cycle profiles in more sophisticated models.} We will not explain the concept of the stationary distribution here, but cover it below in Life-Cycle Model 5. To compute the stationary distribution we need one other piece of information, namely how many households are of each age, we will just assume there is an equal fraction of each age, so $1/J$ agents of each age; it is almost always assumed that the total population is normalized to have a mass of 1.\footnote{How households are distributed is needed to compute the stationary distribution, but is not relevant to the life-cycle profiles themselves (the life-cycle profile command essentially ignores them). This looks a bit silly in this simple example as we have to set up information we don't need, but it will make sense in more advanced models.}
...
%
\subsection{Life-Cycle model 5: Earnings are hump-shaped (and explain agent distribution)}
...
\paragraph{The agent distribution.}
Computing life-cycle profiles internally requires what is called the 'agent distribution' (also called the 'stationary distribution'). The agent distribution is the joint distribution of households over the state space: the endogenous states ($a$ in this model) and age $j$. It tells you what fraction of all households are at each point in the model; the total mass of households is assumed to equal one.

We have to state where households start in period one, and this is done by creating $jequaloneDist$ (the distribution of agents at age $j=1$) which should be of mass one. The $StationaryDist\_{}FHorz\_{}Case1()$ command in VFI Toolkit will then follow households from one period to the next based on where they were and what choices (Policy) they make (and, in later models, the exogenous shock transitions). Doing this we get the distribution at ages $2, 3, \ldots, J$. We also need to define how many households are of each age; we provide this via the parameter $\mu_j$ ($Params.mewj$ in code), a vector of weights summing to one. Here we just assume an equal mass $1/J$ of each age.

In this deterministic model with all households born identically, every household of a given age is at the same point on $a$, so the distribution at each age is a single point mass. The agent distribution will only start doing meaningful work once we add idiosyncratic shocks in Life-Cycle Model 8.
