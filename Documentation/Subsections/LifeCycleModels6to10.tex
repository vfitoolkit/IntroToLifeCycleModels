...
The warm glow of bequests from leaving behind assets $a$ is, $warmglow(a)=wg1 \frac{(1+a/wg2)^{1-wg3}}{1-wg3}$; where $wg2$ controls the degree to which bequests are a luxury good ($\geq 1$; $=1$ would be a normal good), and $wg1$ determines the importance of the warm glow of bequests (relative to, e.g., consumption or leisure). Notice how the functional form is almost exactly the same as the utility of consumption, and $wg3$ acts like the curvature parameter. By using the same functional form it is much easier to choose an appropriate calibration of the parameters for the warm-glow of bequests.

In the empirical data, wealthy households leave bigger requests (even as a proportion of their wealth) than low wealth households. This is why we chose a functional form for the warm-glow of bequests that treats the bequests as a luxury good.
...
We will set $wg2=3$, which makes bequests a luxury good. You can see the impact this has on assets when old by looking at the life-cycle profile of assets.

\subsection{Life-Cycle model 8: Idiosyncratic shocks and heterogeneity (and explain agent distribution)}
...
Now that we have an idiosyncratic shock $z$, the agent distribution is non-trivial. Households of the same age but with different shock histories end up at different $(a, z)$ states, so the distribution at each age is no longer a point mass.

As in Life-Cycle Model 5, we have to state where households start by setting $jequaloneDist$ (here, $0.7$ employed and $0.3$ unemployed, all with zero assets, total mass one). The $StationaryDist\_{}FHorz\_{}Case1()$ command will follow households from one period to the next based on where they were, what choices (Policy) they make, and now also the exogenous shock transitions $\pi_z$. Doing this we get the distribution at ages $2, 3, \ldots, J$. Now that we have conditional survival probabilities $s_j$ (introduced in Life-Cycle Model 6), the mass of households of age $j+1$ is $s_j$ times the mass at age $j$; we encode this in $\mu_j$ ($Params.mewj$ in code): start with mass one at age $1$, push forward using $s_j$, then normalize so the total mass is one.

The result is $StationaryDist$, a 3D array of size $[n_a, n_z, N_j]$ which represents the fraction of all households at each $(a, z, j)$, with entries summing to one. Almost everything we will compute from a model with heterogeneous agents---means, age-conditional statistics, Gini coefficients, life-cycle profiles---is an integral against this distribution.

From here on out, you will see lines turning on \textit{vfoptions.divideandconquer} and \textit{vfoptions.gridinterplayer} in most of the codes; these make the value function iteration faster and more accurate, and are explained in Life-Cycle Models 29 and 30.
