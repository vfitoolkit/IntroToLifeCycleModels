...
This Intro to Life-Cycle Models shows lots of examples, which are very useful to go through to understand how specific models are put together with VFI Toolkit. Many users will prefer to first check out the \href{https://www.vfitoolkit.com/2025-workshop-lse/}{VFI Toolkit workshop}, which focuses more on the concepts around how VFI Toolkit thinks of models, and many of the options you will want to use. The workshop includes video and codes that walk through models and discuss big picture concepts as well as talking about options for how you should set up your models.

The series of examples here builds up to a life-cycle model in which people make consumption-savings and consumption-leisure choices, which has working age and retirement, which is capable of capturing that earnings are hump-shaped over age (peaking around ages 45-55), the variance of both income and consumption increase with age, incomes grow in line with deterministic economic growth of the economy as a whole, people have some assets left when they die, people face the risk of substantial medical costs when old, and where borrowing constraints and precautionary savings play an important role. And we will be able to use these to plot life-cycles, including mean, variance, and gini coefficients conditional on age, and even on 5 year age-bins. We will also be easily able to simulate panel data sets from the model on which we could run regressions. To get an idea of the more advanced Life-Cycle models, check the Table of Contents two pages later.

These life-cycle models can be used easily requiring very little knowledge of numerical methods; all you need is Matlab and a gpu.\footnote{The codes make automatic use of the gpu via the VFI Toolkit for Matlab; the gpu must be an NVIDIA gpu. I tested all of these codes on a gpu with 8gb of ram, vast majority of them would work with less gpu memory but no promises :)} They are intended as a simple way to understand and use life-cycle models. Follow-up documents will then build on these to explain how to extend them to general equilibrium OLG (overlapping-generation) models, and how to solve general equilibrium transition paths for OLG models. The codes are an 'introduction' to life-cycle models, not the final word. For models with one or two endogenous states VFI Toolkit can solve the majority of models used in the literature, but not quite all of them. If you find this interesting and want to start working with models that have three or more assets you will need to learn to write (much faster) code yourself; I recommend looking up the EGM (endogenous grid method) for solving finite-horizon models and starting from there.\footnote{The VFI Toolkit default is to use pure discretization (discretize all decision variables, next period endogenous state, this period endogenous state, and the exogenous states), this is simple and robust, but not nearly as good a combination of speed and accuracy and more sophisticated methods. But it also allows you to turn on ---which you essentially always should--- 'divide-and-conquer' and 'grid interpolation layer' which together make it powerful.} Obviously from the perspective of the codes there is no need to stick to 'hours worked', 'assets', and 'labor productivity shocks' as the decision variable, endogenous state, and exogenous state, they can be anything you want to call them/interpret them as.
...
\input{./Subsections/LifeCycleModels1to5.tex}

\input{./Subsections/LifeCycleModels6to10.tex}

\input{./Subsections/LifeCycleModels11to14.tex}

\newpage
\section{Illustrating some important concepts}
\input{./Subsections/LifeCycleModels15to19.tex}

\newpage
\section{Further building up Life-Cycle Models}
\input{./Subsections/LifeCycleModels20to25.tex}

\input{./Subsections/LifeCycleModels26to30.tex}

\newpage
\section{Solving Models Faster!}
\input{./Subsections/LifeCycleModels29to30.tex}

\newpage
\section{Portfolio-Choice in Life-Cycle Models (riskyasset)} \label{sec:portfoliochoice}
\input{./Subsections/LifeCycleModels31to35.tex}

\newpage
\section{Behavioural Life-Cycle Models} \label{sec:behavioural}
\input{./Subsections/LifeCycleModels36to39.tex}

\newpage
\section{Additional Endogenous States} \label{sec:endostates}
\input{./Subsections/LifeCycleModels40to44.tex}

\newpage
\section{Estimating Life-Cycle Models} \label{sec:estimation}
\input{./Subsections/LifeCycleModels45to50.tex}
...
\newpage
\input{./Appendix/LifeCycleModelAppendix_ExogenousShocks.tex}
...
\input{./Appendix/LifeCycleModelAppendix_EpsteinZin.tex}

\newpage
\subsection{Quasi-Hyperbolic Discounting}
\input{./Appendix/LifeCycleModelAppendix_QuasiHyperbolic.tex}

\newpage
\subsection{Temptation and Self-Control: Gul-Pesendorfer preferences}
\input{./Appendix/LifeCycleModelAppendix_GulPesendorfer.tex}

\newpage
\subsection{Ambiguity Aversion}
\input{./Appendix/LifeCycleModelAppendix_AmbiguityAversion.tex}

\newpage
\input{./Appendix/LifeCycleModelAppendix_GMMestimation.tex}
