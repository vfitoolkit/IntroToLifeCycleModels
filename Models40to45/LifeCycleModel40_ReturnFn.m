function F=LifeCycleModel40_ReturnFn(aprime,hprime,a,h,z,kappaj,r,tau_p,theta,upsilon,sigma,gamma,phi,alpha,delta_k,delta_o,delta_r,agej,Jr,b)
...
elseif hprime>0
    % Budget constraint
    c=(1+r)*a+(1-tau_p)*earnings+(1-delta_o)*h-tau_hhprime+(agej>=Jr)*b-aprime-hprime;
    if c>0
        % utility function
        if upsilon==0
