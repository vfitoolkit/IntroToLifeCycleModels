%% Life-Cycle Model 38: Loss Aversion
% Alter life-cycle model 10 to have loss-aversion

% There are essentially two parts to using loss aversion.
% 1. Set up the loss-aversion parameters
% 2. Set up a 'residual asset' for the lag of consumption (an endogenous state that is determined 'residually' by all the other variables)
% 3. Set up the return function appropriately

% Lag of consumption forms the reference point for loss-aversion

% 1. The loss-aversion parameters are
...
% That is all. Every other line of code is unchanged!! Loss aversion really is that easy ;)
...
% Grid sizes to use
n_d=0;
n_a=[201,101]; % Endogenous asset holdings, lag of consumption (residual endogenous state)
...
vfoptions.rprimeFn=@(aprime,a,z,w,agej,Jr,pension,r,kappa_j) LifeCycleModel38_ConsumptionLagFn(aprime,a,z,w,agej,Jr,pension,r,kappa_j); % consumption (today, as this is then next period's lag of consumption)
simoptions.rprimeFn=vfoptions.rprimeFn;
% Because we need all the grids to evaluate rprimeFn these have to be
% passed via simoptions
...
xlabel('lag of consumption')
ylabel('Assets (a)')
subplot(3,1,2); surf(asset_grid*ones(1,n_a(2)),ones(n_a(1),1)*clag_grid',V(:,:,zind,50))
title('Value function: median value of z, age 50')
xlabel('lag of consumption')
ylabel('Assets (a)')
subplot(3,1,3); surf(asset_grid*ones(1,n_a(2)),ones(n_a(1),1)*clag_grid',V(:,:,zind,80))
title('Value function: median value of z, age 80')
xlabel('lag of consumption')
ylabel('Assets (a)')
...
title('Life Cycle Profile: Consumption')
