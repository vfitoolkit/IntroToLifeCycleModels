...
% rates are higher for the young, so we will make the transitions change
% towards more stable employment as agents age. [The actual numbers are
% rough, not properly based on empirical evidence.] We will keep the same
...
% Because we have the age-dependent transition matrix on the exogenous
% shocks z, we can see that their distribution is changing with age, as
% seen by the changing 'fraction unemployment'. 
% Note that we started all households in what was the stationary
% distribution for the pi_z at agej=1, so if pi_z did not change then the
% fraction unemployed would have been constant in age.
