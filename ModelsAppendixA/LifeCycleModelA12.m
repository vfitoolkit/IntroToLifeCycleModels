%% Life-Cycle Model A12: Parametrize Exogenous Shocks - ExogShockFn and EiidShockFn
% Use two exogenous shocks: z and e, persistent and transitory shocks to labor efficiency units, respectively

% This is just the same model as Life-Cycle Model 11
% Only change is that instead of creating z_grid and pi_z, we set up
% vfoption.ExogShockFn which takes in parameters and outputs z_grid and
% pi_z. This 'parameterizes' the exogenous markov shocks, allowing us to
% calibrate/estimate them, or determine them in general eqm.
% We do the same for the i.i.d. shocks using vfoptions.EiidShockFn

% Most of the code is unchanged.
% Around line 90 the code that used to create z_grid and pi_z has been
% replaced by vfoptions.ExogShockFn (and create placeholders z_grid=[] and
% pi_z=[]). Just below that the code that used to create e_grid and pi_e
% has been replaced by vfoptions.EiidShockFn.
% We also need to set 
% simoptions.ExogShockFn=vfoptions.ExogShockFn;
% simoptions.EiidShockFn=vfoptions.EiidShockFn;
...
Params.n_z=n_z; % put in Params so we can input this to vfoptions.ExogShockFn
vfoptions.ExogShockFn=@(rho_z,sigma_epsilon_z,n_z) LifeCycleModelA12_ExogShockFn(rho_z,sigma_epsilon_z,n_z);
simoptions.ExogShockFn=vfoptions.ExogShockFn;
% Placeholders
z_grid=[];
pi_z=[];
...
Params.n_e=n_e; % put in Params so we can input this to vfoptions.EiidShockFn
vfoptions.EiidShockFn=@(sigma_epsilon_e,n_e) LifeCycleModelA12_EiidShockFn(sigma_epsilon_e,n_e);
simoptions.EiidShockFn=vfoptions.EiidShockFn;
